...
void main()
{
	...
	if(useBones)
	{
		mat4 boneTransform = boneMatrices[int(boneIds[0])] * boneWeights[0];
		boneTransform += boneMatrices[int(boneIds[1])] * boneWeights[1];
		boneTransform += boneMatrices[int(boneIds[2])] * boneWeights[2];
		boneTransform += boneMatrices[int(boneIds[3])] * boneWeights[3];
	...
	gl_Position = projection * view * model * vec4(localPos.xyz, 1);
};
