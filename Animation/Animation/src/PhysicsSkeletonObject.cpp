...
PhysicsSkeletonObject::PhysicsSkeletonObject(const std::string& path, bool debugModel)
{
	LoadModel(path, debugModel);
}

void PhysicsSkeletonObject::SetAnimatedPosition(const glm::vec3& position)
{
	transform.SetPosition(position);
}

void PhysicsSkeletonObject::SetAnimatedRotation(const glm::vec3& rotation)
{
	transform.SetRotation(rotation);
}

void PhysicsSkeletonObject::SetAnimatedRotation(const glm::quat& rotation)
{
	transform.SetQuatRotation(rotation);
}

void PhysicsSkeletonObject::SetAnimatedScale(const glm::vec3& scale)
{
	transform.SetScale(scale);
}

void PhysicsSkeletonObject::SetBaseColor(const glm::vec4& color)
{
...
void PhysicsSkeletonObject::DrawShaded(MeshAndMaterial* mesh, Shader* shader)
{
	shader->Bind();
...
void PhysicsSkeletonObject::CalcualteNodeMatricses(RootNodeInfo* meshRootNodeInfo, HeirarchyNode* node, glm::mat4& parentTransformationMatrix,
...
void PhysicsSkeletonObject::AnimateNodes(float deltaTime)
{
	for (MeshAndMaterial* mesh : meshes)
	{
...
void PhysicsSkeletonObject::PlayAnimation(const std::string& name)
{
